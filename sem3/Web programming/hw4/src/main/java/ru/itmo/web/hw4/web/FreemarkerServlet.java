...
public class FreemarkerServlet extends HttpServlet {
    ...
    private final int BigIntegerException = -1;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (request.getRequestURI() != null && !request.getRequestURI().equals("/") &&
            ...
            try {
                ...
                template = freemarkerConfiguration.getTemplate(URLDecoder.decode("/404", UTF_8) + ".ftlh");
            }
        } else {
            uri = "/index";
            template = freemarkerConfiguration.getTemplate(URLDecoder.decode("/index", UTF_8) + ".ftlh");
        }
    ...
    private Map<String, Object> getData(HttpServletRequest request, String realUri) {
        ...
        for (Map.Entry<String, String[]> e : request.getParameterMap().entrySet()) {
            if (e.getValue() != null && e.getValue().length == 1) {
                if (e.getKey().endsWith("_id")) {
                    if (isNumeric(e.getValue()[0])) {
                        try {
                            data.put(e.getKey(), Long.parseLong(e.getValue()[0], 10));
                        } catch (NumberFormatException ignored) {
                            data.put(e.getKey(), BigIntegerException);
                        }
                    }
                } else {
        ...
        String[] uriPartition = realUri.split("/");
        data.put("page_title", uriPartition[uriPartition.length - 1]);
